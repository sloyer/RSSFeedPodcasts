  ...
  try {
    ...
    for (const feedConfig of feeds) {
      try {
        ...
        const shouldBypassETag = testMode || (feedConfig.last_fetched 
          ? (Date.now() - new Date(feedConfig.last_fetched).getTime()) > (12 * 60 * 60 * 1000)
          : true); // Always fetch if never fetched before
        
        // In test mode or time-based bypass, skip ETag checks to ensure we get fresh data
        const headers = { 'User-Agent': 'RSS Aggregator/1.0' };
        
        if (!shouldBypassETag && feedConfig.last_etag) {
          headers['If-None-Match'] = feedConfig.last_etag;
        }
        if (!shouldBypassETag && feedConfig.last_modified) {
