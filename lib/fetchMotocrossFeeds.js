      ...
      try {
        ...
        for (const item of items) {
          try {
            ...
            // Strategy E: Fetch og:image from the article page when the feed provides no images
            // Used for feeds like MotoOnline that have text-only RSS items
            if (!imageUrl && articleUrl) {
              try {
                ...
                if (pageRes.ok) {
                  ...
                  if (ogMatch) {
                    const ogImg = ogMatch[1].replace(/&amp;/g, '&').trim();
                    if (ogImg && !shouldSkipImage(ogImg)) {
                      imageUrl = ogImg;
                      imageSource = 'og-image-fetch';
                    }
                  }
                }
              } catch (fetchErr) {
                // Silent fail — og:image fetch is best-effort only
              }
            }
