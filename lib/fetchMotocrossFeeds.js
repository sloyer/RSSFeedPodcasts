  ...
  try {
    ...
    for (const feedConfig of feeds) {
      try {
        ...
        const headers = { 
          'User-Agent': 'Mozilla/5.0 (compatible; RSSFeedPodcasts/1.0; +https://rss-feed-podcasts.vercel.app)',
        ...
        for (const item of items) {
          try {
            ...
            if (!imageUrl) {
              ...
              const shouldSkipImage = (url) => {
                ...
                const skipPatterns = [
                  ...
                  'tld-', 'troy-lee', 'troyleedesigns', // TLD brand ads (all variations)
                  '980x250', '728x90', '300x250', '1476x200', // Common ad banner sizes
                  '_banner', '-banner', '/banner',
                  'wolf.jpg', 'moto-master', 'brake', // Specific MX Vice recurring ads
                  '-ad-', '_ad_', '/ad/', '-ls-', // Common ad naming patterns
                  'sponsor', 'promo', // Ad-related keywords
                  // GateDrop specific
                  'cropped-simple-professional', // GateDrop favicon
                  '-32x32', '-150x150' // Common thumbnail/icon sizes
                ];
            ...
            if (imageUrl) {
              ...
              // Strip Drupal image style prefix to get full-size image
              // e.g. vitalmx.com/styles/thumbnail_cropped_235x132/s3/... → vitalmx.com/s3/...
              imageUrl = imageUrl.replace(/\/styles\/[^/]+\//, '/');
