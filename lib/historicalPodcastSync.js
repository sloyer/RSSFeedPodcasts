...
async function historicalPodcastSync(dryRun = false) {
  ...
  for (const feedConfig of HISTORICAL_FEEDS) {
    try {
      console.log(`🎯 Processing: ${feedConfig.description}`);
      ...
      for (let i = 0; i < items.length; i += RATE_LIMIT.batchSize) {
        ...
        try {
          ...
          if (!dryRun && batchData.length > 0) {
            ...
            if (newEpisodes_batch.length > 0) {
              ...
              if (insertError) {
                ...
                console.log(`  ✅ Inserted ${newEpisodes_batch.length} new episodes, ${duplicates_batch} duplicates`);
              }
            } else {
              duplicates += duplicates_batch;
              console.log(`  ⏭️ All ${duplicates_batch} episodes already exist`);
            }
          } else if (dryRun) {
          ...
          if (i + RATE_LIMIT.batchSize < items.length) {
            console.log(`  ⏳ Waiting ${RATE_LIMIT.delayBetweenBatches}ms before next batch...`);
