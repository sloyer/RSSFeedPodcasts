...
async function fetchNewTweets(bearerToken, account) {
  ...
  // Build author lookup map (covers both parent and quoted tweet authors)
  const authorMap = {};
  ...
  // Build quoted tweets lookup map
  const quotedTweetsMap = {};
  if (data.includes?.tweets) {
    for (const qt of data.includes.tweets) {
      quotedTweetsMap[qt.id] = qt;
    }
  }
  ...
  for (const tweet of data.data) {
    ...
    // Build quoted tweet object if this tweet quotes another
    let quotedTweet = null;
    const quotedRef = tweet.referenced_tweets?.find(r => r.type === 'quoted');
    if (quotedRef) {
      const qt = quotedTweetsMap[quotedRef.id];
      if (qt) {
        const qtAuthor = authorMap[qt.author_id] || {};
        let qtImageUrl = null;
        if (qt.attachments?.media_keys?.length > 0) {
          qtImageUrl = mediaMap[qt.attachments.media_keys[0]] || null;
        }
        quotedTweet = {
          twitter_id: qt.id,
          text: qt.text,
          ...
          author_avatar: qtAuthor.avatar || null,
          image_url: qtImageUrl,
          tweet_url: `https://x.com/${qtAuthor.username}/status/${qt.id}`
        };
      }
    }

    // Strip the trailing t.co link Twitter appends when quoting a tweet
    let tweetText = tweet.text;
    if (quotedTweet) {
      tweetText = tweetText.replace(/\s*https:\/\/t\.co\/\S+$/, '').trim();
    }
