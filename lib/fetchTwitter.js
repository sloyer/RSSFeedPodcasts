...
async function scrapeAllAccounts() {
  ...
  try {
    ...
    // Scrape each account (with delays)
    for (const account of accounts) {
      await randomDelay(1000, 2000); // Delay between accounts
      const tweets = await scrapeTweetsFromUser(browser, context, account.username, 5);
