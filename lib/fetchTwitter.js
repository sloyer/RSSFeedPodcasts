    ...
    for (const instruction of instructions) {
      ...
      for (const entry of entries) {
        try {
          ...
          const tweetAuthorUsername = userLegacy?.screen_name || '';
          
          // STRICT FILTER: Only include tweets actually authored by this account
          if (tweetAuthorUsername.toLowerCase() !== username.toLowerCase()) {
            continue; // Skip - this tweet is from someone else (retweet, reply, etc)
          }
          
          // Skip retweets (double check)
          ...
          const userName = userLegacy?.name || username;
  ...
  try {
    ...
    for (const account of accounts) {
      try {
        ...
        if (tweets.length > 0) {
          // Filter to recent tweets and ensure they have avatars
          const recentTweets = tweets.filter(tweet => {
            const tweetDate = new Date(tweet.posted_at);
            return tweetDate >= cutoffDate;
          }).map(tweet => ({
            ...tweet,
            // Use avatar from account info if tweet didn't have one
            author_avatar: tweet.author_avatar || avatarMap.get(account.username) || ''
          }));
