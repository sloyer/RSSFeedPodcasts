...
async function fetchTweetsFromUser(username, count = 5) {
  try {
    ...
    for (const tweet of timeline.slice(0, count)) {
      try {
        // Skip if no valid tweet ID
        if (!tweet.tweet_id) {
          continue;
        }
        
        // Skip retweets
        if (tweet.retweets > 0 && tweet.text?.startsWith('RT @')) {
          continue;
        }
        ...
        // Fix avatar URL
        let fixedAvatar = authorAvatar;
        if (fixedAvatar) {
          fixedAvatar = fixedAvatar.replace('http://', 'https://');
          fixedAvatar = fixedAvatar.replace('_normal.', '_400x400.');
        }
  ...
  try {
    ...
    for (const account of accounts) {
      try {
        ...
        if (tweets.length > 0) {
          // Filter to recent tweets (author info already in tweets from twitter-api45)
