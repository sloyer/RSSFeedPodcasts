// lib/mxgpScraper.js
// Scrapes racerxonline.com for MXGP race results and championship standings.
//
// URL patterns on Racer X:
//   /mxgp/{year}/races              → all events + session links for the season
//   /mxgp/{year}/points/{cls}       → championship standings
//   /mxgp/{year}/{event}/{cls}      → class overall results (moto scores per rider)
//   /mxgp/{year}/{event}/{cls}/{session} → individual moto/session results
    ...
    try {
      ...
      for (const node of graph) {
        if (node['@type'] === 'SportsEvent') {
          const slug = (node.url || '').replace(`${RX_BASE}/mxgp/${year}/`, '').split('/')[0];
          if (!slug) continue;
          eventMeta[slug] = {
            name: node.name || '',
