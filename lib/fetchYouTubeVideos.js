  ...
  try {
    ...
    for (const channel of channels) {
      try {
        ...
        // New channels (never fetched) get a 30-day backfill regardless of daysBack
        const effectiveDays = (!channel.last_video_id && !channel.last_fetched) ? Math.max(daysBack, 30) : daysBack;
        const newVideos = await fetchChannelVideos(channel, channel.last_video_id, effectiveDays);
