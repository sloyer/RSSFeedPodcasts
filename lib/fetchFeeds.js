  ...
  try {
    ...
    for (const feedConfig of feeds) {
      try {
        ...
        // Add timeout to RSS parsing (bypass ETag if needed)
        const feedPromise = shouldBypassETag 
          ? parser.parseURL(feedConfig.feed_url) // Force fresh fetch
          : parser.parseURL(feedConfig.feed_url); // Normal fetch (with ETag from parser)
